<template>
  <aside class="app-sidebar" :class="{ collapsed: collapsed }">
    ...
    <div v-if="wikiStore.loadingTree" class="loading-placeholder">加载中…</div>
    <div v-else id="tree-root">
      ...
      <div v-else-if="!wikiStore.tree" style="padding:10px;color:var(--text-muted);font-size:0.75rem;text-align:center;">
        知识库未配置
      </div>
    </div>
  </aside>
